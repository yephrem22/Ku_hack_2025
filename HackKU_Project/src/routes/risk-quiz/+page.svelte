  ...
  <div class="max-w-2xl mx-auto my-12 px-6 py-8 bg-white rounded-xl shadow-lg font-sans">
    ...
    {#each questions as question}
        <div class="mb-6">
            ...
            <textarea bind:value={riskAnswerMap[question]} class="w-full p-3 border border-gray-300 rounded-lg resize-y focus:ring-2 focus:ring-red-500 focus:outline-none" rows="4"></textarea>
        </div>
    {/each}
  
    <div class="flex justify-center space-x-4">
        <button on:click={submitAnswers} class="px-6 py-3 bg-red-600 text-white font-semibold rounded-lg hover:bg-red-700 focus:outline-none">Save Answers</button>
        <button on:click={sendToGemini} class="px-6 py-3 bg-red-600 text-white font-semibold rounded-lg hover:bg-red-700 focus:outline-none">Analyze Responses</button>
    </div>
    ...
    <div class="flex justify-center mt-6">
        <button on:click={() => goto('/dashboard')} class="px-6 py-3 bg-gray-500 text-white font-semibold rounded-lg hover:bg-gray-600 focus:outline-none">Back to Dashboard</button>
    </div>
  </div>
